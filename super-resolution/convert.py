...
def convert(input_path, output_path, converter, extensions):
    ...
    for extension in extensions:
        ...
        img_paths_ext = glob.glob(os.path.join(input_path, '**', '*.{}'.format(extension)), recursive=True)
...
def numpy_converter(out_dir, img_id, img):
    ...
    arr_path = os.path.join(out_dir, '{}.npy'.format(img_id))
...
def jpeg_converter(quality):
    def converter(out_dir, img_id, img):
        ...
        jpg_path = os.path.join(out_dir, '{}.jpg'.format(img_id))
