...
class DIV2KSequence(Sequence):
    def __init__(self,
        ...
        if not os.path.exists(path):
            raise FileNotFoundError("Path {} doesn't exist".format(path))
        ...
        if downgrade not in DOWNGRADES:
            ...
            raise ValueError("downgrade must be in {}".format(DOWNGRADES))
    ...
    def _hr_image_path(self, id):
        ...
        #return os.path.join(self.path, 'DIV2K_{}_HR'.format(self.subset), format(id, '04d') + '.npy')
        return os.path.join(self.path, 'cell_stain_{}_HR'.format(self.subset), format(id, '04d') + '.npy')
    ...
    def _lr_image_path(self, id):
        ...
        #return os.path.join(self.path, 'DIV2K_{}_LR_{}'.format(self.subset, self.downgrade), 'X{}'.format(self.scale), format(id, '04d') + 'x{}.npy'.format(self.scale))
        return os.path.join(self.path, 'cell_stain_{}_LR_{}'.format(self.subset, self.downgrade), 'X{}'.format(self.scale), format(id, '04d') + 'x{}.npy'.format(self.scale))
