...
1. In `pix2pix.py`, change the generator loss from `reduce_mean` to `reduce_sum`.

2. In `pix2pix.py`, change the saver to save the snapshots from 1 to unlimited, `max_to_keep=1` to `max_to_keep=0`

3. In `tools\process.py`, allowing paired image names to have different suffixs in function `combine`.

4. Add `reduce_model.py`, which will reduce the trained model so that we can use an image tensor as input.

5. Add `freeze_model.py`, which freezes the reduced model to a single Tensorflow .pb file.
