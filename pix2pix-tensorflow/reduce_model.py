...
# Hongbing: The ngf and ndf should be parameters 
#ngf = 32
#ndf = 32
...
def process_image(x):
    ...
    def transform(image):
        ...
        if width == CROP_SIZE:
            ...
            r = tf.image.resize_images(image, [CROP_SIZE, CROP_SIZE], method=tf.image.ResizeMethod.AREA)
        else:
            r = tf.image.resize_images(image, [CROP_SIZE, CROP_SIZE], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        """
        r = tf.image.resize_images(image, [CROP_SIZE, CROP_SIZE], method=tf.image.ResizeMethod.AREA)
...
# Hongbing: Redefine the create_model without targets
...
def generate_output(x):
    with tf.name_scope('generate_output'):
        ...
        test_inputs = process_image(x)

        # inputs and targets are [batch_size, height, width, channels]
        #model = create_model(test_inputs, test_targets)
        model = create_model(test_inputs)
...
if __name__ == '__main__':
    ...
    x = tf.placeholder(tf.uint8, shape=(256, 256, 3), name='image_tensor')  # input tensor
    y = generate_output(x)
